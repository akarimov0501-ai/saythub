    ...
    <section id="featured-section">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        ...
        <div className="flex items-center gap-2">
          <div className="bg-slate-100 dark:bg-slate-800/80 p-1 rounded-full flex items-center gap-1 border border-slate-200/50 dark:border-slate-700/60">
            <button 
              onClick={() => setTab('new')} 
          ...
          <button 
            ...
            className="text-sky-600 dark:text-sky-400 hover:text-sky-700 dark:hover:text-sky-300 font-semibold text-xs sm:text-sm flex items-center gap-1 group ml-2 transition cursor-pointer"
          >
      ...
      {websites.length === 0 ? (
        <div className="py-16 text-center bg-white dark:bg-slate-900/60 rounded-2xl border border-slate-100 dark:border-slate-800/80 shadow-sm">
          ...
          <h3 className="text-lg font-bold text-slate-700 dark:text-slate-200 mb-1">No websites found</h3>
          <p className="text-sm text-slate-500 dark:text-slate-400 mb-4">Try adjusting your search keywords or clear current filters.</p>
          <button 
            onClick={resetFilters} 
                ...
                <div>
                  <div className="flex items-start justify-between mb-3.5">
                    <div className="flex items-center gap-3.5">
                      ...
                      <div>
                        <h3 className="font-bold text-slate-900 dark:text-slate-100 text-base leading-tight group-hover:text-sky-600 dark:group-hover:text-sky-400 transition-colors flex items-center gap-1.5">
                          {site.name}
                          <ExternalLink className="w-3.5 h-3.5 opacity-0 group-hover:opacity-100 transition-opacity text-slate-400 dark:text-slate-500" />
                        </h3>
                    ...
                    <button 
                      ...
                      className={`p-2 rounded-xl text-slate-400 dark:text-slate-500 hover:text-sky-600 dark:hover:text-sky-400 hover:bg-slate-50 dark:hover:bg-slate-800 transition ${
                        isBookmarked ? 'text-sky-600 dark:text-sky-400 bg-sky-50 dark:bg-sky-950/60' : ''
                      }`}
                    >
                      <Bookmark className={`w-5 h-5 ${isBookmarked ? 'fill-sky-600 text-sky-600 dark:fill-sky-400 dark:text-sky-400' : ''}`} />
                    </button>
                  </div>
